...
function upload_done_process()
{
    ...
    if (
    urlParams.has("uploaded") &&
    urlParams.get("uploaded") === "true" &&
    urlParams.has("slug") &&
    urlParams.get("slug") !== "" &&
    urlParams.has("page") &&
    urlParams.get("page") === "awesome-google-review"
    ) {
        setTimeout(function () {
            var newUrl =
            ajax_object.main_site_url +
            "/wp-admin/edit.php?business=" +
            urlParams.get("slug") +
...
let btnProcess_BUSINESS_START = $(
    "#google_review_upload_form .btn-process.job_start"
);
let btnProcess_BUSINESS_CHECK = $(
    "#google_review_upload_form .btn-process.check_start"
);
...
let btnProcess_BUSINESS_UPLOAD = $(
    "#google_review_upload_form .btn-process.upload_start"
);
...
function initial_check()
{
    ...
    $.ajax({
        ...
        success: function (response) {
            if (
            response.data.btn_check == 0 &&
            response.data.btn_check_status == 1 &&
            response.data.btn_start == 1 &&
            response.data.btn_upload == 0
            ) {
          ...
          // start 1
            if (
            response.data.btn_start &&
            !response.data.btn_check &&
            !response.data.btn_upload
            ) {
          ...
          // url: 'http://localhost:3000/search/',
            ...
            success: function (response) {
                ...
                if (response.success === 1) {
                    ...
                    $(".google_review_upload_form span.wrong-sign")
                    .addClass("firm_area_sign")
                    .hide();
                    $(".google_review_upload_form span.correct-sign")
                    .addClass("firm_area_sign")
                    .show();
                    ...
                    .empty()
                    .append(
                        `<div class="search--txt">No results found <div class="close-button" onclick="closePopup()">x</div></div>`
                    );
                    $(".google_review_upload_form span.correct-sign")
                    .addClass("firm_area_sign")
                    .hide();
            ...
            error: function (xhr, status, error) {
                ...
                $(".search-result")
                .empty()
                .append(`<div class="search--txt">API Call Failed: ${error}</div>`);
            },
            complete: function () {
                $("#processbar").hide();
...
$("#google_review_upload_form button.job_start").click(function (event) {
    ...
    if (firm_name.trim() != "") {
        ...
        Swal.fire({
            ...
            text:
            "Are you certain about initiating this job? Once completed, you'll be able to upload reviews.",
...
$("#google_review_upload_form button.check_start").click(function (event) {
    ...
    Swal.fire({
        title: "GET",
        html:
        "Let's begin gathering reviews for " +
        `<b>${$(FirmNameInput).val()}</b>` +
        " !",
        showCloseButton: true,
        allowOutsideClick: false,
        confirmButtonColor: "#405640",
...
$("#google_review_upload_form button.upload_start").click(function (event) {
    ...
    Swal.fire({
        ...
        html:
        "Initiate the uploading process for " +
        `<b>${$(FirmNameInput).val()}</b>` +
        " !",
        showCloseButton: true,
        allowOutsideClick: false,
...
function review_delete_process(check, $this)
{
    ...
    if (selected_value == 0) {
        ...
        Swal.fire({
            title: "Delete Review?",
            html:
            "Initiate deletion of the review data for " +
            `<b>${selected_value_name}</b>` +
            " !",
            showCloseButton: true,
            allowOutsideClick: false,
